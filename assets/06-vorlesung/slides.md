...
.center[![reflection](../img/reflection.gif)]
...
At its core, reflection builds on `java.lang.Class`, a generic class that _is_ the definition of classes.
...
- use the `?` (wildcard) and appropriate bounds for _any_ type.
...
You can distinguish between _declared_ fields (methods, ...), and "just" fields: `.getFields()` (and `.getMethods()` etc.) will return _the public_ fields of an object, including those inherited by base classes.
